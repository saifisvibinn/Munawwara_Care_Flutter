...
public class SwiftFlutterCallkitIncomingPlugin: NSObject, FlutterPlugin, CXProviderDelegate {
    ...
    public func provider(_ provider: CXProvider, perform action: CXAnswerCallAction) {
        ...
        // Also mark the call's own data so `activeCalls()` reports isAccepted=true
        // (self.data is a separate instance from the call stored in callManager).
    ...
    public func provider(_ provider: CXProvider, perform action: CXEndCallAction) {
        guard let call = self.callManager.callWithUUID(uuid: action.callUUID) else {
            if(self.answerCall == nil && self.outgoingCall == nil){
                ...
                if let appDelegate = UIApplication.shared.delegate as? CallkitIncomingAppDelegate {
                    appDelegate.onDeclineIncomingWithoutManagedCall(
                        extra: self.data?.extra as? [String: Any]
                    )
                }
            } else {
